...
function Movies({ Data, bookmarked, handleBookmark }: DataProps) {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      transition={{ duration: 0.5 }}
      className="relative lg:px-[2.3rem] md:px-[1.5rem] px-[1rem] pt-[3rem] pb-[4rem]"
    >
      ...
      <ul className="lg:gap-[2rem] md:gap-[1.5rem] grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-[1rem]">
        {Data.filter((item) => item.category === "Movie").map((item, index) => (
